...
export function executeHook(rule, event, hookEvent) {
  ...
  if ('run' in rule) {
    ...
    for (const cmd of cmds) {
      ...
      if (rule.async) {
        const child = spawn(expanded, { shell: true, env, cwd: projectDir, detached: true, stdio: 'ignore' });
        child.unref();
        continue;
      }

      const result = spawnSync(expanded, { shell: true, env, cwd: projectDir, timeout, maxBuffer: 1024 * 1024 });
